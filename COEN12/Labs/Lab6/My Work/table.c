...
/* Quicksort Function Implemented by Luke Manzitto */
...
static int partition(SET *sp, void **array, int low, int high){ /* Worst Case Runtime: O(n) */
	int i,j;
	assert(sp!=NULL);
	void *temp;
	void *pivot = array[high];
	i = low - 1;
	for(j=low;j<high;j++){ /* partitions the array and returns the pivot location */
...
void quicksort(SET *sp, void **array, int low, int high){ /* Worst Case Runtime: O(n^2) */
	assert(sp!=NULL);
	if(low<high){ /* Recursively sorts the array using the partition function */
...
void *getElements(SET *sp){ /* Worst Case Runtime: O(n^2) */
	int i,j;
	assert(sp!=NULL);
	void **array;
	array=malloc(sizeof(void*)*sp->count);
	assert(array!=NULL);
	for(i=0,j=0;i<sp->length;i++){ /* Copies data into a new array */
		if(sp->flags[i]==FILLED){
			array[j++]=sp->data[i];
		}
	}
	quicksort(sp,array,0,sp->count-1); /* Sorts data in new array */
