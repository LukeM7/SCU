...
GLuint sunTexture, earthTexture, moonTexture, shuttleTexture;
GLuint vao[numVAOs];
GLuint vbo[numVBOs];
...
Sphere sphere = Sphere(48);
ImportedModel myModel("shuttle.obj");

float toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void setupVertices(void) {
	// Code taken from class example for 6.1
	...
	std::vector<float> model_pvalues;
	std::vector<float> model_tvalues;
	std::vector<float> model_nvalues;

	for (int i = 0; i < myModel.getNumVertices(); i++) {
		model_pvalues.push_back((modelVert[i]).x);
		model_pvalues.push_back((modelVert[i]).y);
		model_pvalues.push_back((modelVert[i]).z);
		model_tvalues.push_back((modelTex[i]).s);
		model_tvalues.push_back((modelTex[i]).t);
		model_nvalues.push_back((modelNorm[i]).x);
		model_nvalues.push_back((modelNorm[i]).y);
		model_nvalues.push_back((modelNorm[i]).z);
	}
	...
	// I was too lazy to make a separate VAO for the Import Model, I'm assuming that would be a better solution
	...
	glBufferData(GL_ARRAY_BUFFER, model_pvalues.size() * 4, &model_pvalues[0], GL_STATIC_DRAW);

	glBindBuffer(GL_ARRAY_BUFFER, vbo[4]);
	glBufferData(GL_ARRAY_BUFFER, model_tvalues.size() * 4, &model_tvalues[0], GL_STATIC_DRAW);

	glBindBuffer(GL_ARRAY_BUFFER, vbo[5]);
	glBufferData(GL_ARRAY_BUFFER, model_nvalues.size() * 4, &model_nvalues[0], GL_STATIC_DRAW);
}
...
void display(GLFWwindow* window, double currentTime) {
	...
	// ------------------------ shuttle around sun
	mvStack.push(mvStack.top());
	mvStack.top() *= glm::translate(glm::mat4(1.0f), glm::vec3(0.0, sin((float)currentTime) * 2.0, cos((float)currentTime) * 2.0));
	//mvStack.push(mvStack.top());
	mvStack.top() *= glm::rotate(glm::mat4(1.0f), (float)currentTime, glm::vec3(-1.0, 0.25, 0.0));
	...
	mvStack.top() *= scale(glm::mat4(1.0f), glm::vec3(0.75f, 0.75f, 0.75f));
	glUniformMatrix4fv(mvLoc, 1, GL_FALSE, glm::value_ptr(mvStack.top()));
	glBindBuffer(GL_ARRAY_BUFFER, vbo[0]);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(0);
